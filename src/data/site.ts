...
export const hero = {
  image: {
    ...
    alt: 'Цех порошковой покраски с производственной линией'
  },
  title: 'Порошковая покраска металла под ключ — от единичных изделий до серийных партий',
  subtitle:
    'Выполняем полный цикл в одном месте: пескоструй, обезжиривание, нанесение порошка, полимеризация и контроль покрытия по ТЗ.',
  benefits: ['Камера до 6,5 м', 'Срок от 48 часов', 'Гарантия до 24 месяцев'],
...
export const advantages = {
  section: {
    ...
    title: 'Почему предприятия выбирают ПрофПорошок',
    subtitle:
      'Даём предсказуемый результат по толщине и адгезии покрытия, работаем по договору и соблюдаем производственные сроки.'
  },
  cards: [
    {
      value: '12+',
      title: 'лет опыта',
      description: 'С 2014 года красим детали для машиностроения, строительства, торговли и частных заказчиков.'
    },
    {
      value: '2500+',
      title: 'заказов в год',
      description: 'Отработанные процессы позволяют стабильно выдерживать сроки даже в сезон высокой загрузки.'
    },
    {
      value: '3 этапа',
      title: 'контроля качества',
      description: 'Проверяем подготовку поверхности, равномерность нанесения и итоговую полимеризацию покрытия.'
    },
    {
      value: '24/7',
      title: 'поддержка B2B',
      description: 'Для корпоративных клиентов закрепляем менеджера и согласуем график отгрузок под ваш цикл.'
    }
  ]
} as const;
